    ...
    var supplier_id=document.getElementById("supplier_id").value.trim();
  	if(from_date == "")
  	 ...
  	 if(to_date == "")
        ...
        $.post("account_payable_report",{supplier_id:supplier_id,customer_id:customer_id,view_all:view_all,from_date:from_date,to_date:to_date},function(result){
          //alert(result);
            setTimeout(function() {
             $("#tbodypayable").empty().append(result);     
             $(".overlay").remove();
            }, 0);
           }); 
