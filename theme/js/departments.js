...
$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#department-form')[0]);//form name
						/*Check XSS Code*/
						...
						$.ajax({
						type: 'POST',
						url: base_url+'departments/newdepartment',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
//Delete Record start
function delete_department(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("departments/delete_department",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
