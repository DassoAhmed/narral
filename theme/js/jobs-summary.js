...
$('#save,#update').click(function (e) {
	...
	check_field("total_qty");
	check_field("actitvty_code");
    ...
    if(this_id=="save")  //Save start
    {
					...
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#jobs-update-form')[0]);//form name
						/*Check XSS Code*/
             ...
             // alert(result);//return;
             			result=result.trim();
							if(result=="success")
							{
								window.location=base_url+"manufacturing";
								return;
							}
				...
				//e.preventDefault
					...
					if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'manufacturing/update_items',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
              ...
              //alert(result);return;
              			result=result.trim();
							if(result=="success")
							{
								...
								window.location=base_url+"manufacturing/job_list";
							}
							else if(result=="failed")
							{
							   toastr["error"]("Sorry! Failed to save Record.Try again!");
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: 'manufacturing/multi_delete_jobs',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
