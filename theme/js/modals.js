...
$(".add_brand").click(function(e){
    ...
    var this_id=this.id;
			if(confirm("Are you Sure to save Brand ?")){
				
				e.preventDefault();
				data = new FormData($('#brands-form')[0]);//form name
				/*Check XSS Code*/
				if(!xss_validation(data)){ return false; }
				...
				success: function(result){
					...
					{   
						$('#brands-modal').modal('toggle');
					   	// var newOption = '<option value='+data.id+' selected>'+data.brand_name+'</option>';
...
$(".add_category").click(function(e){
    ...
    var this_id=this.id;
			if(confirm("Are you Sure to save category ?")){
