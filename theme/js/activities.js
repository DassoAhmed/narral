...
var base_url=$("#base_url").val().trim();
 ...
 //Validate Input box or selection box should not be blank or empty
  check_field("activity_name");
  check_field("activity_date");
  check_field("fin_item_id");
...
for(var n=1;n<=rowcount;n++){
  ...
  var this_id=this.id;
    ...
    if(confirm("Do You Wants to Save Record ?")){
      e.preventDefault();
      data = new FormData($('#activities-form')[0]);//form name
      /*Check XSS Code*/
      ...
      $.ajax({
      type: 'POST',
      url: base_url+'manufacturing/activity_save_and_update?command='+this_id+'&rowcount='+rowcount+'&tot_subtotal_amt='+tot_subtotal_amt+'&tot_discount_to_all_amt='+tot_discount_to_all_amt+'&tot_round_off_amt='+tot_round_off_amt+'&tot_total_amt='+tot_total_amt+"&other_charges_amt="+other_charges_amt,
      data: data,
      cache: false,
      contentType: false,
      processData: false,
      ...
      result=result.split("<<<###>>>");
        if(result[0]=="success")
        {
          location.href=base_url+"manufacturing";
        }
...
$.post(base_url+"manufacturing/return_row_with_data/"+rowcount+"/"+item_id,{},function(result){
      //alert(result);
      $('#manufacturing_table tbody').append(result);
