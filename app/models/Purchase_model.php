...
class Purchase_model extends CI_Model {
	...
	public function verify_save_and_update(){
		...
		for($i=1;$i<=$rowcount;$i++){ 
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$unit_id = $q0->row()->id;
				if($operator == '/'){
					$unit = ($purchase_qty / $q0->row()->operator_value);
					$new_purchase_rice = ($price_per_unit * $q0->row()->operator_value);
				}else{
					$unit= ($purchase_qty * $q0->row()->operator_value);
					$new_purchase_rice = ($price_per_unit / $q0->row()->operator_value);
				}
				$purchaseitems_entry = array(
		    				...
		    				'item_id' 			=> $item_id,
		    				'purchase_qty' 		=> $unit,
		    				'price_per_unit' 	=> $new_purchase_rice,
		    				'tax_id' 			=> $tax_id,
		    				'unit_id' 			=> $unit_id,
		    				'tax_amt' 			=> $tax_amt,
		    				'tax_type' 			=> $tax_type,
				...
				$q6=$this->db->query("update db_items set stock ='$new_stock' where id ='$item_id'");
	...
	public function return_purchase_list($purchase_id){
		...
		foreach ($q1->result() as $res1) {
			$q2=$this->db->query("select item_name,stock,tax_type,sales_unit_id,price from db_items where id=".$res1->item_id);
			$q3=$this->db->query("select * from db_tax where id=".$res1->tax_id)->row();
			$q4=$this->db->query("select * from db_units where id=".$q1->row()->unit_id)->row();

			$info['item_id'] = $res1->item_id;
			$info['item_name'] = $q2->row()->item_name;
			if($q4->operator == '/'){
				$info['item_purchase_qty'] = $res1->purchase_qty * $q4->operator_value;
				$info['item_available_qty'] = ($q2->row()->stock * $q4->operator_value). ' ' .$q4->short_name;
				$info['item_price'] = $q2->row()->price / $q4->operator_value;
				$info['item_purchase_price'] = $q2->row()->price / $q4->operator_value;
			}else{
				$info['item_purchase_qty'] = $res1->purchase_qty / $q4->operator_value;
				$info['item_available_qty'] = $q2->row()->stock / $q4->operator_value;
				$info['item_price'] = $q2->row()->price * $q4->operator_value;
				$info['item_purchase_price'] = $q2->row()->price * $q4->operator_value;
			}
		// $info['item_available_qty'] = $q1->row()->stock;
			...
			$info['sales_unit_id'] = $q2->row()->sales_unit_id;
			$info['item_tax'] = $q3->tax;
			$info['item_tax_name'] = $q3->tax_name;
	...
	public function return_row_with_data($rowcount,$info){
		...
		?>
			...
			<!-- </td> -->
			   ...
			   <input type="text" name="td_data_<?=$rowcount;?>_10" id="td_data_<?=$rowcount;?>_10" class="form-control text-right no-padding only_currency text-center" readonly value="<?=$item_unit_cost;?>">
			</td>
		  ...
		  <div class="modal-dialog ">
		    ...
		    <div class="row">
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" onclick="save_payment(<?=$purchase_id;?>)" class="btn bg-green btn-lg btn-lg payment_save">Save<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
