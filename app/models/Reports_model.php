...
class Reports_model extends CI_Model {
	...
	public function show_sales_due_report(){
		...
		$this->db->select("a.id,a.sales_code,a.sales_date,b.customer_name,b.sales_due,b.customer_code,a.grand_total,a.paid_amount");
		...
		$this->db->where("(b.`sales_due` > 0 and b.`id`= a.`customer_id`)");
		$this->db->from("db_sales as a");
		$this->db->where("a.`sales_status`= 'Final' and a.payment_status != 'Paid'");
		$this->db->from("db_customers as b");
	...
	public function show_item_chart_of_acc_sales_report(){
		...
		$this->db->select("b.id,b.sales_code,b.sales_code, a.payment_date, a.payment_type, a.payment, a.payment_note");

		if($momo=="yes"){
			$this->db->where("(a.payment_date>='$from_date' and a.payment_date<='$to_date')");
		}
			$this->db->from("db_salespayments as a,db_sales as b");
			$this->db->where("b.id = a.sales_id and a.payment_type = 'Momo'");
			// $this->db->from("");
			$q2=$this->db->get();
		if($q2->num_rows()>0){
			$i=0;
			...
			foreach ($q2->result() as $res2) {
				...
				echo "<td>".show_date($res2->payment_date)."</td>";
				echo "<td class='text-right'>".app_number_format($res2->payment)."</td>";
				echo "<td class='text-right'>".$res2->payment_type."</td>";
				// echo "<td class='text-right'>".app_number_format($res1->grand_total-$res1->paid_amount)."</td>";
				echo "</tr>";
				$tot_grand_total+=$res2->payment;
				// $tot_paid_amount+=$res2->payment;
				// $tot_due_amount+=($res1->grand_total-$res1->paid_amount);
	...
	public function show_item_cash_sales_report(){
		...
		if($cash=="yes"){
			...
			$this->db->from("db_salespayments as a,db_sales as b");
			$this->db->where("b.id = a.sales_id and a.payment_type = 'Cash'");
			// $this->db->from("");
			$q2=$this->db->get();
		if($q2->num_rows()>0){
			$i=0;
	...
	public function show_item_bank_sales_report(){
		...
		if($bank=="yes"){
			...
			$this->db->from("db_salespayments as a,db_sales as b");
			$this->db->where("b.id = a.sales_id and a.payment_type = 'Bank/Cheque'");
			// $this->db->from("");
			$q2=$this->db->get();
		if($q2->num_rows()>0){
			$i=0; 
	...
	public function show_purchase_due_report(){
		extract($_POST);
	    ...
	    // $purchase_id = $this->db->query("SELECT a.purchase_code,a.purchase_date,'b.payment_date','b.payment' from ")->row()->purchase_id;
		if($supplier_id!=''){
		...
		$this->db->where("b.`id`= a.`supplier_id` and purchase_due !=0");
		$this->db->from("db_purchase as a");
		$this->db->where("a.`purchase_status`= 'Received' and a.payment_status != 'Paid'");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td><a id='purchase_num' title='View Invoice' href='".base_url("purchase/invoice/$res1->id")."'>".$res1->purchase_code."</a></td>";
	...
	public function show_item_momo_purches_report(){
		...
		$this->db->select("b.id,b.purchase_code,a.payment_date, a.payment_type, a.payment, a.payment_note");

		if($momo=="no"){
			$this->db->where("(a.payment_date>='$from_date' and a.payment_date<='$to_date')");
		}
			$this->db->from("db_purchasepayments as a,db_purchase as b");
			$this->db->where("b.id = a.purchase_id and a.payment_type = 'Momo'");
			// $this->db->from("");
			$q2=$this->db->get();
		if($q2->num_rows()>0){
			$i=0;
	...
	public function show_item_cash_purches_report(){
		...
		if($cash=="no"){
			...
			$this->db->from("db_purchasepayments as a,db_purchase as b");
			$this->db->where("b.id = a.purchase_id and a.payment_type = 'Cash'");
			// $this->db->from("");
			$q2=$this->db->get();
		if($q2->num_rows()>0){
			$i=0;
	...
	public function show_item_bank_purches_report(){ 
		...
		if($bank=="yes"){
			...
			$this->db->from("db_purchasepayments as a,db_sales as b");
			$this->db->where("b.id = a.purchase_id and a.payment_type = 'Cash'");
			// $this->db->from("");
			$q2=$this->db->get();
		if($q2->num_rows()>0){
	...
	public function show_item_momo_expense_report(){
		extract($_POST);
		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
		...
		// $this->db->where("c.`payment_type`= a.`payment_type`");
		$this->db->from("db_expense as a");
		$this->db->from("db_expense_category as b");
		// $this->db->from("db_paymenttypes as c");
		...
		if($q2->num_rows()>0){
			...
			foreach ($q2->result() as $res2) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td>".$res2->expense_code."</td>";
				echo "<td>".$res2->expense_date."</td>";
				echo "<td>".$res2->category_name."</td>";
				echo "<td>".$res2->reference_no."</td>";
				echo "<td>".$res2->expense_for."</td>";
				echo "<td class='text-right'>".app_number_format($res2->expense_amt)."</td>";
				echo "<td>".$res2->payment_type."</td>";
				echo "<td>".$res2->note."</td>";
				echo "<td>".ucfirst($res2->created_by)."</td>";
				echo "</tr>";
				$tot_expense_amt+=$res2->expense_amt;
			}
			echo "<tr>
	...
	public function show_stock_report(){
		...
		$this->db->where("b.id=a.tax_id and active_path ='active'");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$investment_value = $res1->purchase_price * $res1->stock;
				echo "<tr>";
				echo "<td>".++$i."</td>";
				...
				echo "<td class='text-right'>".app_number_format($investment_value)."</td>";
				...
				$tot_inv_value +=$investment_value;
			}
			...
			echo "<tr>
					  ...
					  <td class='text-right text-bold'>".app_number_format($tot_inv_value)."</td>
	...
	// Booking Section
	public function booking_customer_due_report(){
		...
		$this->db->select("a.id,a.booking_code,a.booked_date,b.sales_due,b.customer_name,b.customer_code,a.grand_total,a.paid_amount");
	    
		
		if($view_all=="no"){
			$this->db->where("(a.booked_date>='$from_date' and a.booked_date<='$to_date')");
		}
		$this->db->where("b.`id`= a.`customer_id` AND b.`sales_due` > 0");
		$this->db->from("db_booking as a");
		$this->db->where("a.`booking_status`= 'Final' and a.payment_status != 'Paid'");
		$this->db->from("db_customers as b");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td><a title='View Invoice' href='".base_url("booking/invoice/$res1->id")."'>".$res1->booking_code."</a></td>";
				echo "<td>".show_date($res1->booked_date)."</td>";
				echo "<td>".$res1->customer_name."</td>";
				echo "<td class='text-right'>".app_number_format($res1->grand_total)."</td>";
				echo "<td class='text-right'>".app_number_format($res1->paid_amount)."</td>";
				echo "<td class='text-right'>".app_number_format($res1->grand_total-$res1->paid_amount)."</td>";
	...
	public function booking_report(){
		...
		$this->db->select("a.id,a.booking_code,a.booked_date,b.customer_name,b.customer_code,a.grand_total,a.paid_amount");
		...
		$this->db->where("b.`id`= a.`customer_id`");
		$this->db->from("db_booking as a");
		$this->db->where("a.`booking_status`= 'Final'");
		$this->db->from("db_customers as b");
	...
	public function show_booking_payments_report(){
		extract($_POST);
		
		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
		
		$this->db->select("c.id,c.booking_code,c.booked_date,b.customer_name,b.customer_code,a.payment_type,a.payment_note,a.payment,a.payment_date");
		...
		$this->db->where("c.id=a.booking_id");

		$this->db->from("db_bookingpayments as a");
		$this->db->from("db_customers as b");
		$this->db->from("db_booking as c");
		$this->db->where("c.`booking_status`= 'Final'");
	...
	public function booking_customer_payments_report(){
		extract($_POST); 
	...
	public function show_item_cash_booking_report(){
		...
		$this->db->select("b.id,b.booking_code, a.payment_date, a.payment_type, a.payment, a.payment_note");
		...
		if($payment_type!=''){
			...
			$this->db->from("db_bookingpayments as a,db_booking as b");
			$this->db->where("b.id = a.booking_id");
		...
		if($q2->num_rows()>0){
			...
			foreach ($q2->result() as $res2) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td><a title='View Invoice' href='".base_url("booking/invoice/$res2->id")."'>".$res2->booking_code."</a></td>";
				echo "<td>".show_date($res2->payment_date)."</td>";
				echo "<td class='text-right'>".app_number_format($res2->payment)."</td>";
				echo "<td class='text-right'>".$res2->payment_type."</td>";
				echo "</tr>";
	...
	public function sales_payment_type_report(){
		...
		$this->db->select("b.id,b.sales_code, a.payment_date, a.payment_type, a.payment, a.payment_note");
	...
	public function account_receivable_report(){
	...
	public function account_payable_report(){
		extract($_POST);
		 
		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
		
		$this->db->select("a.id,a.purchase_code,a.purchase_date,b.supplier_name,b.supplier_code,b.purchase_due, a.grand_total,a.paid_amount");
	    // $purchase_id = $this->db->query("SELECT a.purchase_code,a.purchase_date,'b.payment_date','b.payment' from ")->row()->purchase_id;
		if($supplier_id!=''){
			$this->db->where("a.supplier_id=$supplier_id");
		}
	...
	public function accaunt_pay_rec_booking_report(){
		...
		$this->db->select("a.id,a.booking_code,a.booked_date,b.customer_name,b.customer_code,b.sales_due,a.payment_status,a.grand_total,a.paid_amount");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td><a title='View Invoice' href='".base_url("sales/invoice/$res1->id")."'>".$res1->booking_code."</a></td>";
				echo "<td>".show_date($res1->booking_date)."</td>";
				echo "<td>".$res1->customer_code."</td>";
				echo "<td>".$res1->customer_name."</td>";
				echo "<td class='text-right'>".app_number_format($res1->grand_total)."</td>";
				echo "<td class='text-right'>".app_number_format($res1->paid_amount)."</td>";
	...
	public function accaunt_pay_rec_booking_order_report(){
		extract($_POST);
		 
		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
		
		$this->db->select("a.id,a.order_code ,a.order_date,b.supplier_name,b.supplier_code,b.purchase_due, a.grand_total,a.paid_amount");
	    // $purchase_id = $this->db->query("SELECT a.purchase_code,a.purchase_date,'b.payment_date','b.payment' from ")->row()->purchase_id;
		if($supplier_id!=''){
			$this->db->where("a.supplier_id=$supplier_id");
		}
		if($view_all=="no"){
			$this->db->where("(a.order_date>='$from_date' and a.order_date<='$to_date')");
		}
		$this->db->where("b.`id`= a.`supplier_id` and purchase_due !='0'");
		$this->db->from("db_booking_orders as a");
		$this->db->where("a.`order_status`= 'Received' and a.`payment_status` !='paid'");
		$this->db->from("db_suppliers as b");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td><a id='purchase_num' title='View Invoice' href='".base_url("purchase/invoice/$res1->id")."'>".$res1->order_code."</a></td>";
				echo "<td>".show_date($res1->order_date)."</td>";
				echo "<td>".$res1->supplier_code."</td>";
				echo "<td>".$res1->supplier_name."</td>";
				echo "<td class='text-right'>".app_number_format($res1->grand_total)."</td>";
				echo "<td class='text-right'>".app_number_format($res1->paid_amount)."</td>";
