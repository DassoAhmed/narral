...
class Order_model extends CI_Model {
	...
	//Datatable start
	var $table = 'db_booking_orders as a';
	var $column_order = array( 'a.id','a.order_date', 'a.delivery_date','a.order_qty','a.order_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.supplier_name','a.paid_amount','a.order_status','a.return_bit'); //set column field database for datatable orderable
	var $column_search = array('a.id','a.order_date','a.delivery_date','a.order_qty','a.order_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.supplier_name','a.paid_amount','a.order_status','a.return_bit'); //set column field database for datatable searchable 
	...
	//Save Cutomers
	public function verify_save_and_update_order(){
	    ...
	    if($command=='save'){//Create purchase code unique if first time entry
			...
			$order_init=$q5->row()->purchase_init;

			$this->db->query("ALTER TABLE db_booking_orders AUTO_INCREMENT = 1");
			$q4=$this->db->query("select coalesce(max(id),0)+1 as maxid from db_booking_orders");
			$maxid=$q4->row()->maxid;
			$order_code=$order_init.str_pad($maxid, 4, '0', STR_PAD_LEFT);
		    ...
		    $order_entry = array(
							...
							'order_qty' 			=> $total_briolers,
		    				'order_status' 			=> $purchase_status,
		    				'supplier_id' 			=> $supplier_id,
		    				'total_brol ' 			=> $total_briolers,
		    				'total_lay' 			=> $total_layers,
			...
			$q1 = $this->db->insert('db_booking_orders', $order_entry);
			...
			$q1 = $this->db->where('id',$purchase_id)->update('db_booking_orders', $order_entry);

			$q11=$this->db->query("delete from db_booking_orderitems where order_id='$purchase_id'");
			if(!$q11){
				return "failed";
			}
		}
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$q2 = $this->db->insert('db_booking_orderitems', $purchaseitems_entry);
				if($q2){
					$q7=$this->db->query("UPDATE db_bookeditems set order_id ='$purchase_id' where order_status = 'inactive'");
					if($q7){
						$q7=$this->db->query("UPDATE db_bookeditems set order_status ='active' where order_status = 'inactive'");
		...
		if($amount>0 && !empty($payment_type)){
			...
			$q3 = $this->db->insert('db_booking_orderpayments', $purchasepayments_entry);
			
		}
		
		$q10=$this->update_order_payment_status($purchase_id,$supplier_id);
		if($q10!=1){
			return "failed";
		}
	...
	public function delete_payment($payment_id){
        $this->db->trans_begin();
		$purchase_id = $this->db->query("select order_id from db_booking_orderpayments where id=$payment_id")->row()->purchase_id;

		$q1=$this->db->query("delete from db_booking_orderpayments where id='$payment_id'");

		$supplier_id = $this->db->query("select supplier_id from db_booking_orders where id=$purchase_id")->row()->supplier_id;

		$q2=$this->update_order_payment_status($purchase_id,$supplier_id);
		if($q1!=1 || $q2!=1)
		{
	...
	public function update_order_payment_status_by_order_id($purchase_id,$supplier_id){
		$q8=$this->db->query("select COALESCE(SUM(payment),0) as payment from db_booking_orderpayments where order_id='$purchase_id'");
		$sum_of_payments=$q8->row()->payment;
		

		$q9=$this->db->query("select coalesce(sum(grand_total),0) as total from db_booking_orders where id='$purchase_id'");
		...
		$q7=$this->db->query("update db_booking_orders set 
		...
		$purchase_due =$this->db->query("select COALESCE(SUM(grand_total),0)-COALESCE(SUM(paid_amount),0) as purchase_due from db_booking_orders where supplier_id=$supplier_id and order_status='Received'")->row()->purchase_due;
	...
	function update_order_payment_status($purchase_id,$supplier_id){
		if(!$this->update_order_payment_status_by_order_id($purchase_id,$supplier_id)){
			return false;
		}
		return true;
	}
	...
	function fun_temp_update_suppliers_purchase_due(){
		...
		if($q11->num_rows()>0){
			foreach ($q11->result() as $res) {
				...
				$q12=$this->db->query("select id,supplier_id from db_booking_orders where supplier_id=".$res->id);
				if($q12->num_rows()>0){
					foreach ($q12->result() as $res12) {
						if(!$this->purchase_model->update_order_payment_status($res12->id,$res12->supplier_id)){
							return false;
						}
					}
				}
	...
	public function delete_purchase($ids){
		...
		foreach ($q11->result() as $res11) {
			$q2=$this->update_order_payment_status($res11->id,$res11->supplier_id);
			if(!$q2){ return "failed";}
		}
	...
	public function return_purchase_list($purchase_id){
		$q1=$this->db->select('*')->from('db_booking_orderitems')->where("order_id=$purchase_id")->get();
		$rowcount =1;
		foreach ($q1->result() as $res1) {
			...
			$info['item_purchase_qty'] = $res1->order_qty;
			$info['item_tax'] = $q3->tax;
			$info['item_tax_name'] = $q3->tax_name;
