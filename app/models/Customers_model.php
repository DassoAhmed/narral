...
class Customers_model extends CI_Model {
	...
	public function show_customer_profile_modal($customer_id){
	    ...
	    $q16 = $this->db->query("select coalesce(sum(grand_total),0) as total_sales_amount,coalesce(sum(paid_amount),0) as total_paid_amount from db_sales where customer_id=$customer_id"); 
	    $total_sales_amount = $q16->row()->total_sales_amount;
	    $total_paid_amount = $q16->row()->total_paid_amount;
	    //$total_sales_due_amount =$total_sales_amount - $total_paid_amount;
	    $due_amount = number_format($total_sales_amount - $total_paid_amount,2,'.','') ;
		$customer_state = $this->db->query("SELECT a.grand_total, a.paid_amount,a.created_date, b.item_name, ");

		?>
		<div class="modal fade" id="customer_profile">
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		          <span aria-hidden="true">&times;</span></button>
		        <h4 class="modal-title text-center">Customers Transactions Detail</h4>
		      </div>
		      <div class="modal-body">
		      	...
		      	<div class="row invoice-info">
			        ...
			        <div class="col-sm-12 invoice-col">
			        	...
			        	<table class="table table-sm table-bordered bg-info" width="100%">
			        		...
			        		<tr>
			        			<td class="text-right"><?= $this->lang->line('sales_due'); ?></td>
			        			<td class="text-right"><?= $CI->currency($due_amount); ?></td>
			        		</tr>
