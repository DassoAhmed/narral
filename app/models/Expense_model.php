...
class Expense_model extends CI_Model {
	...
	var $column_order = array('a.id','a.expense_date','b.category_name','a.reference_no','a.expense_for','a.expense_amt','payment_type','a.note','a.created_by'); //set column field database for datatable orderable
	var $column_search = array('a.id','a.expense_date','b.category_name','a.reference_no','a.expense_for','a.expense_amt','payment_type','a.note','a.created_by'); //set column field database for datatable searchable 
	...
	public function verify_and_save(){
		...
		$query1="insert into db_expense(expense_code,category_id,reference_no,expense_for,expense_amt,payment_type,note,created_date,created_time,created_by,status,system_ip,system_name,expense_date)
						values('$expense_code','$category_id','$reference_no','$expense_for','$expense_amt','$payment_type','$note','$CUR_DATE','$CUR_TIME','$CUR_USERNAME',1,'$SYSTEM_IP','$SYSTEM_NAME','".date("Y-m-d",strtotime($expense_date))."')";
	...
	public function update_expense(){
		...
		$query1="update db_expense set category_id='$category_id',expense_date='".date("Y-m-d",strtotime($expense_date))."',reference_no='$reference_no',expense_for='$expense_for',expense_amt='$expense_amt',payment_type='$payment_type',note='$note' where id=$q_id";
		if ($this->db->simple_query($query1)){
