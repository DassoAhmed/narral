...
class Items_model extends CI_Model {
	...
	var $column_order = array( 'a.id','a.item_image','a.item_code','a.item_name','b.category_name','a.lot_number','c.unit_name','a.purchase_price','a.stock','a.alert_qty','a.sales_price','d.tax_name','d.tax','a.status','e.brand_name','a.sales_unit_id','c.operator_value'); //set column field database for datatable orderable
	var $column_search = array( 'a.id','a.item_image','a.item_code','a.item_name','b.category_name','a.lot_number','c.unit_name','a.purchase_price','a.stock','a.alert_qty','a.sales_price','d.tax_name','d.tax','a.status','e.brand_name','a.custom_barcode','a.sales_unit_id','c.operator_value'); //set column field database for datatable searchable 
	...
	private function _get_datatables_query()
	{
		...
		$this->db->join('db_units as c','c.id=a.sales_unit_id','left');
		$this->db->join('db_tax as d','d.id=a.tax_id','left');
		// $this->db->join('db_units as f','f.id=a.sales_unit_id','left');
		$this->db->where("a.active_path = 'active' ");
		$i = 0; 
	...
	public function verify_and_save(){
		...
		$query1="insert into db_items(description,item_code,item_name,brand_id,category_id,sku,unit_id,sales_unit_id, alert_qty,lot_number,expire_date,
							...
							values('$description','$item_code','$item_name','$brand_id','$category_id','$sku','$unit_id','$sales_unit','$alert_qty','$lot_number','$expire_date',
									'$price','$tax_id','$purchase_price','$tax_type',$profit_margin,
									'$sales_price','$custom_barcode',
		...
		// $q70=$this->db->query("select COALESCE(SUM(stock),0) as stock from db_items where item_id='$item_id'");
		// $item_stock=$q7->row()->stock;	
		
		// $stock=($item_stock+$new_opening_stock);
	...
	public function get_details($id,$data){
		...
		if($query->num_rows()==0){
			...
			$data['sales_unit_id']=$query->sales_unit_id;
