...
class Sales_model extends CI_Model {
	...
	var $column_order = array( 'a.return_bit','a.id','a.sales_date','a.sales_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.customer_name','a.paid_amount','a.sales_status','a.other_charges_amt','a.pos'); //set column field database for datatable orderable
	var $column_search = array('sales_due','a.return_bit','a.id','a.sales_date','a.sales_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.customer_name','a.paid_amount','a.sales_status','a.other_charges_amt','a.pos'); //set column field database for datatable searchable 
	...
	public function save_payment(){
		...
		$q4=$this->db->query("select coalesce(max(id),0)+1 as maxid from db_salespayments");
		$maxid=$q4->row()->maxid;
		$payment_code=$sales_init.str_pad($maxid, 4, '0', STR_PAD_LEFT);
		...
		if($amount>0 && !empty($payment_type)){
			...
			$q3 = $this->db->insert('db_salespayments', $salespayments_entry);
			if($q3){
				$q4=$this->db->query("UPDATE db_sales SET new_amt_paid = '$amount',payment_code= '$payment_code' WHERE id='$sales_id'");
