...
class Items extends MY_Controller {
	...
	public function newitems(){
		...
		$this->form_validation->set_rules('unit_id', 'Unit');
		$this->form_validation->set_rules('price', 'Item Price');
		$this->form_validation->set_rules('tax_id', 'Tax');
		$this->form_validation->set_rules('purchase_price', 'Purchase Price');
		//$this->form_validation->set_rules('profit_margin', 'Profit Margin', 'trim|required');
		$this->form_validation->set_rules('sales_price', 'Sales Price');
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo "Please Fill Compulsory( marked with *) Fields.";
		}
	}
	...
	public function ajax_list()
	{
		...
		foreach ($list as $items) {
			...
			$row[] = (!empty($items->item_image) && file_exists($items->item_image)) ? "
						<a style=' border-radius:100%' title='Click for Bigger!' href='".base_url($items->item_image)."' data-toggle='lightbox'>
						<image style='border-radius:100%' src='".base_url(return_item_image_thumb($items->item_image))."' width='100%' style='height: 66px;'> </a>" : "
						<image style='border-radius:100%' src='".base_url()."theme/images/no_image.png' title='Itme Image!' width='50%' style='height: 36px;' >";
			$row[] = $items->item_code;
			$row[] = $items->item_name;
			...
			$row[] = $items->stock / $items->operator_value;
			$row[] = $items->alert_qty;
			$row[] = $items->purchase_price * $items->operator_value;
			$row[] = app_number_format($items->sales_price * $items->operator_value);

			 		if($items->status==1){ 
			 			$str= "<span onclick='update_status(".$items->id.",0)' id='span_".$items->id."'  class='label label-info' style='cursor:pointer'>Active </span>";}
					else{ 
