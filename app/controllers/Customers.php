...
class Customers extends MY_Controller {
	...
	public function due()
	{
		...
		$data=$this->data;
		$data['page_title']=$this->lang->line('customers');
		$this->load->view('due-customers.php',$data);
	}
			...
			$row[] = $str;			
					$str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											if($this->permissions('sales_return_payment_add'))
											$str2.='<li>
												<a title="View Customers Transactions" class="pointer" onclick="customer_profile('.$customers->id.')" >
													<i class="fa fa-fw fa-user text-blue"></i>My Profile
												</a>
											</li>';
	...
	{
		$list = $this->customers->get_datatables_2();
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->customers->count_all(),
						"recordsFiltered" => $this->customers->count_filtered_2(),
						"data" => $data,
				);
		//output to json format
		echo json_encode($output);
	...
	public function show_customer_profile_modal(){
		$this->permission_check_with_msg('sales_return_payment_add');
		$customer_id=$this->input->post('customer_id');
		echo $this->customers->show_customer_profile_modal($customer_id);
	}
	...
	public function customer_due_report(){
		echo $this->reports->customer_due_report();
	}
