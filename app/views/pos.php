...
<div class="wrapper">
  ...
  <div class="content-wrapper" style="<?=$css;?> ;background:#fff">
    ...
    <?php include"modals/modal_sales_alternate_unit.php"; ?>
    <!-- **********************MODALS END***************** -->
    <!-- Main content -->
    <section class="content" stlye="background:#fff">
      <div class="row">
        <!-- left column -->
        <div class="col-md-8">
          ...
          <div class="box box-primary">
              ...
              <div class="row" >
                ...
                <div class="col-md-5">
                  ...
                  <div class="form-group">
                     <select class="form-control select2" id="warehouse_id" name="warehouse_id"  style="width: 105%;" onkeyup="shift_cursor(event,'mobile')">
                          <?php
            ...
            <!-- /.box-header -->
             ...
             <?php include"modals_pos_payment/modal_payments_multi.php"; ?>
              ...
              <!-- /.modal -->
              <!-- **********************MODALS END***************** -->
              <div class="box-body" style="width: 105%;margin-left: -32px;">                
              <div class="row">
                <div class="col-md-6">
                  <div class="input-group">
                    <span class="input-group-addon" title="Customer"><i class="fa fa-user"></i></span>
                     <select class="form-control select2" id="customer_id" name="customer_id"  style="width: 105%;" onkeyup="shift_cursor(event,'expense_for')" >
                        <?php
              ...
              <br>
              <div class="row">
                <div class="col-md-12" style="width: 100%;margin-right: -10px">
                  <div class="form-group">
                    <div class="col-sm-12" style="overflow-y:auto;border:1px solid #337ab7;width: 100%;;" >
                      <table class="table table-condensed table-bordered table-striped table-responsive items_table" style="width: 100%;">
                        <thead class="bg-primary">
                          <th width="15%"><?= $this->lang->line('item_name'); ?></th>
                          <th width="12%"><?= $this->lang->line('stock'); ?></th>
                          <th width="15%"><?= $this->lang->line('quantity'); ?></th>
                          <th width="10%"><?= $this->lang->line('price_including_tax'); ?></th>
                          <th width="10%"><?= $this->lang->line('block_total_price'); ?></th>
                          <th width="15%"><?= $this->lang->line('retail_qty'); ?></th>
                          <th width="7%"><?= $this->lang->line('reatail_price'); ?></th>
                          <th width="10%"><?= $this->lang->line('retail_total'); ?></th>
                          <th width="15%"><?= $this->lang->line('total'); ?></th>
              ...
              <div class="box-footer bg-gray" style="width: 105%;margin-left: -32px;">
                <div class="row">
...
function addrow(id){
    ...
    var short_name         =$('#div_'+id).attr('data-item-short-name');
    ...
    var operator_value     =$('#div_'+id).attr('data-item-operator-value');
    var item_unit_id     =$('#div_'+id).attr('data-item-item-unit-id');
    var sales_price_temp=sales_price;
        sales_price     =(parseFloat(sales_price)).toFixed(2);
 
    var quantity        ='<div class="input-group input-group-xs"><span class="input-group-btn"><button style="margin-right: -16px;margin-left: -7px;" onclick="decrement_qty('+item_id+','+rowcount+')" type="button" class="btn btn-default btn-sm"><i class="fa fa-minus text-danger"></i></button></span>';
        ...
        quantity       +='<span class="input-group-btn"><button style="margin-right: -6px;margin-left: -7px;" onclick="increment_qty('+item_id+','+rowcount+')" type="button" class="btn btn-default btn-sm"><i class="fa fa-plus text-success"></i></button></span></div>';
        
        var retail_quantity        ='<div class="input-group input-group-sm"><span class="input-group-btn"><button style="margin-right: -16px;margin-left: -7px;" onclick="decrement_retail_qty('+item_id+','+rowcount+')" type="button" class="btn btn-default btn-sm"><i class="fa fa-minus text-danger"></i></button></span>';
        retail_quantity       +='<input typ="text" value="0" class="form-control no-padding text-center" onkeyup="item_retail_qty_input('+item_id+','+rowcount+')" id="item_retail_qty_'+item_id+'" name="item_retail_qty_'+item_id+'">';
        retail_quantity       +='<span class="input-group-btn"><button style="margin-right: -6px;margin-left: -7px;" onclick="increment_retail_qty('+item_id+','+rowcount+')" type="button" class="btn btn-default btn-sm"><i class="fa fa-plus text-success"></i></button></span></div>';
    ...
    var str=' <tr id="row_'+rowcount+'" data-row="0" data-item-id='+item_id+'>';/*item id*/
        str+='<td style="width:10%;" id="td_'+rowcount+'_0"><a data-toggle="tooltip" title="Click to Change Tax" class="pointer" id="td_data_'+rowcount+'_0" onclick="show_sales_item_modal('+rowcount+')">'+ item_name     +'</a> <i onclick="show_sales_item_modal('+rowcount+')" class="fa fa-edit pointer"></i></td>';/* td_0_0 item name*/ 
        // str+='<td style="width:15%;" id="td_'+rowcount+'_1"></td>';/* td_0_1 item available qty*/
        str+='<td style="width:10%;" id="td_'+rowcount+'_1"><a data-toggle="tooltip" title="Click For Alternate Unit" class="pointer" id="td_'+rowcount+'_0" onclick="show_sales_alternate_unit_modal('+rowcount+')">'+ stock +' '+short_name+'</a> <i onclick="show_sales_alternate_unit_modal('+rowcount+')" class="fa fa-edit pointer"></i></td>';/* td_0_0 item name*/ 
        ...
        retail_info='<input style="width: 60px;margin: -2px -5px 0px -5px;" id="retail_sales_price_'+rowcount+'" onblur="set_to_original('+rowcount+','+item_cost+')" onkeyup="update_price('+rowcount+','+item_cost+')" name="retail_sales_price_'+rowcount+'" type="text" class="form-control no-padding " value="'+sales_price+'">';
        str+='<td id="td_'+rowcount+'_3" class="text-right">'+ info   +'</td>';/* td_0_3 item sales price*/
        /*Tax amt*/
        str+='<td id="td_'+rowcount+'_14" class="text-right"><input data-toggle="tooltip" title="Total" id="td_data_'+rowcount+'_4" name="td_data_'+rowcount+'_4" type="text" class="form-control no-padding pointer" readonly value="'+sub_total+'"></td>';/* td_0_4 item sub_total */
        str+='<input data-toggle="tooltip" title="Click to Change" id="td_data_'+rowcount+'_11" onclick="show_sales_item_modal('+rowcount+')" name="td_data_'+rowcount+'_11" type="hidden" class="form-control no-padding pointer" readonly value="'+tax_amt+'">';
        
        str+='<td id="td_'+rowcount+'_12">'+ retail_quantity+'</td>';/* td_0_2 item available qty*/
        str+='<td id="td_'+rowcount+'_13" class="text-right">'+ retail_info   +'</td>';/* td_0_3 item sales price*/
        str+='<td id="td_'+rowcount+'_14" class="text-right"><input data-toggle="tooltip" title="Total" id="td_data_re_'+rowcount+'_4" name="td_data_re_'+rowcount+'_4" type="text" class="form-control no-padding pointer" readonly value="'+sub_total+'"></td>';/* td_0_4 item sub_total */
        str+='<td id="td_'+rowcount+'_14" class="text-right"><input data-toggle="tooltip" title="Total" id="td_data_total_'+rowcount+'_4" name="td_data_total_'+rowcount+'_4" type="text" class="form-control no-padding pointer" readonly value="0"></td>';/* td_0_4 item sub_total */
        str+='<input data-toggle="tooltip" title="Total" id="td_data_total_qty_'+rowcount+'_4" name="td_data_total_qty_'+rowcount+'_4" type="hidden" class="form-control no-padding pointer" readonly value="0">';/* td_0_4 item sub_total */
        str+='<td id="td_'+rowcount+'_15">'+ remove_btn    +'</td>';/* td_0_5 item gst_amt */
       ...
       // str+='<input type="hidden" id="tr_item_per_'+rowcount+'" name="tr_item_per_'+rowcount+'" value="'+gst_per+'">';
        ...
        str+='<input type="hidden" id="tr_unit_id_'+rowcount+'" name="tr_unit_id_'+rowcount+'" value="'+item_unit_id+'">';
        str+='<input type="hidden" id="tr_tax_value_'+rowcount+'" name="tr_tax_value_'+rowcount+'" value="'+tax_value+'">';
        str+='<input type="hidden" id="tr_operator_value_'+rowcount+'" name="tr_operator_value_'+rowcount+'" value="'+operator_value+'">';
        str+='<input type="hidden" id="tr_input_value_'+rowcount+'" name="tr_input_value_'+rowcount+'" value="0">';
        str+='<input type="hidden" id="tr_block_value_'+rowcount+'" name="tr_block_value_'+rowcount+'" value="0">';
    ...
    //final_total();
    calculate_alternate_value(item_id,rowcount)
...
function set_to_original(row_id,item_cost) {
  ...
  var retail_sales_price=$("#retail_sales_price_"+row_id).val().trim();
  if(sales_price!='' || sales_price==0) {sales_price = parseFloat(sales_price); }
  if(retail_sales_price!='' || retail_sales_price==0) {retail_sales_price = parseFloat(retail_sales_price); }
  ...
  if(retail_sales_price<item_cost){
    toastr["success"]("Default Price Set "+item_price);
    $("#retail_sales_price_"+row_id).parent().removeClass('has-error');
    $("#retail_sales_price_"+row_id).val(item_price);
  }
  make_subtotal($("#tr_item_id_"+row_id).val(),row_id);
  // make_subtotal($("#tr_item_id_"+row_id).val(),row_id);
}
...
//INCREMENT RATAIL ITEM
function increment_retail_qty(item_id,rowcount){
  var item_qty=$("#item_retail_qty_"+item_id).val();
  var stock=$("#td_"+rowcount+"_1").html();
  if(parseFloat(item_qty)<parseFloat(stock)){
    item_qty=parseFloat(item_qty)+1;
    $("#item_retail_qty_"+item_id).val(item_qty);
  }
  make_retail_subtotal(item_id,rowcount);
  calculate_alternate_value(item_id,rowcount)
}
//DECREMENT Reatail ITEM 
function decrement_retail_qty(item_id,rowcount){
  var item_qty=$("#item_retail_qty_"+item_id).val();
  if(item_qty<=1){
    $("#item_retail_qty_"+item_id).val(1);
    return;
  }
  $("#item_retail_qty_"+item_id).val(parseFloat(item_qty)-1);
  make_subtotal(item_id,rowcount);
}
//DECREMENT ITEM 
...
//LEFT SIDE: IF ITEM RETAIL QTY CHANGED MANUALLY
function item_retail_qty_input(item_id,rowcount){
  var item_qty=$("#item_retail_qty_"+item_id).val();
  var stock=$("#td_"+rowcount+"_1").html();
  if(stock==0){
  ...
  if(parseFloat(item_qty)>parseFloat(stock)){
    $("#item_retail_qty_"+item_id).val(stock);
...
function make_subtotal(item_id,rowcount){
  ...
  var retail_sales_price     =$("#retail_sales_price_"+rowcount).val();
  //var gst_per         =$("#tr_item_per_"+rowcount).val();
  var item_qty        =$("#item_qty_"+item_id).val();
  
  var tot_sales_price =parseFloat(item_qty)*parseFloat(sales_price);

  var retail_item_qty        =$("#item_retail_qty_"+item_id).val();
  var total_qty = parseFloat(item_qty)+parseFloat(retail_item_qty) * 1;

  var tot_retail_sales_price =parseFloat(retail_item_qty)*parseFloat(retail_sales_price);
  //var gst_amt=(tot_sales_price * gst_per)/100;


  var subtotal        =parseFloat(tot_sales_price);
  var retail_subtotal        =parseFloat(tot_retail_sales_price);

  //var gst_amt=(tot_sales_price * gst_per)/100;

  console.log("retail_subtotal="+retail_subtotal);
  ...
  retail_subtotal = (tax_type=='Inclusive') ? retail_subtotal : parseFloat(retail_subtotal) + parseFloat(tax_amount);
  var grand_total = (parseFloat(subtotal) + parseFloat(retail_subtotal)*1)
  $("#td_data_"+rowcount+"_4").val(parseFloat(subtotal).toFixed(2));
  $("#td_data_re_"+rowcount+"_4").val(parseFloat(retail_subtotal).toFixed(2));
  $("#td_data_total_"+rowcount+"_4").val(parseFloat(grand_total).toFixed(2));
  $("#td_data_total_qty_"+rowcount+"_4").val(parseFloat(total_qty).toFixed(2));
  console.log("grand_total="+grand_total);
  final_total();
}
function calulate_discount(discount_input,discount_type,total){
  if(discount_type=='in_percentage'){
    return parseFloat(((total + retail_total) * discount_input)/100);
  }
  else{//in_fixed
...
function final_total(){
  ...
  if($(".items_table tr").length>1){
    for(i=0;i<rowcount;i++){
      ...
      retail_total=parseFloat(retail_total)+ + +parseFloat($("#td_data_re_"+i+"_4").val()).toFixed(2);
     
      retail_item_qty=parseFloat(retail_item_qty)+ + +parseFloat($("#item_retail_qty_"+item_id).val()).toFixed(2);
      var new_total =parseFloat(retail_total)+parseFloat(total)*1
    }
    }//for end
  }//items_table
  console.log(new_total);
  total =round_off(total,retail_total);
  // retail_total =round_off(retail_total);
  var discount_amt=0;
  if(total>0 || retail_total>0){
    var discount_amt=calulate_discount(discount_input,discount_type,total);//return value 
  }
  $(".tot_grand ").html(round_off(new_total).toFixed(2));
  set_total(item_qty,discount_amt,total,retail_item_qty,retail_total,total-discount_amt);
}
function set_total(tot_qty=0,tot_disc=0,tot_amt=0,retail_item_qty=0,retail_total=0,  new_total=0){


$(".tot_qty").html(tot_qty + retail_item_qty * 1);
 
  $(".tot_amt   ").html((round_off(tot_amt + retail_total * 1).toFixed(2)));
  $(".tot_disc  ").html((round_off(tot_disc).toFixed(2)));
  $(".tot_grand ").html(round_off(new_total + retail_total * 1).toFixed(2));
}
...
function adjust_payments(){
  ...
  if($(".items_table tr").length>1){
    for(i=0;i<rowcount;i++){
      if(document.getElementById('tr_item_id_'+i)){
      total=parseFloat(total)+ + +parseFloat($("#td_data_"+i+"_4").val()).toFixed(2);
      total=parseFloat(total)+ + +parseFloat($("#td_data_re_"+i+"_4").val()).toFixed(2);
      item_id=$("#tr_item_id_"+i).val();
      retail_item_qty=parseFloat(retail_item_qty)+ + +parseFloat($("#item_retail_qty_"+item_id).val()).toFixed(2);
      item_qty=parseFloat(item_qty)+ + +parseFloat($("#item_qty_"+item_id).val()).toFixed(2);
      }
  ...
  new_total_qty = retail_item_qty + item_qty *1
  if(new_total_qty <=0 ){
  ...
  console.log("retail_item_qty="+retail_item_qty);
  ...
  $(".make_sale").click(function(){
    if(parseFloat(new_total_qty)>parseFloat(stock)){
    $("#item_retail_qty_"+item_id).val(stock);
    toastr["warning"]("Oops! You have only "+stock+" items in Stock");
   // return;
  }
  ...
  if($(".sales_div_tot_qty   ").html(new_total_qty) <= 0){
    toastr["error"]("Please Enter item quantity!! ");
		   	return false;
}else{
$(".sales_div_tot_qty  ").html(new_total_qty);
}
...
$('#order_date,#delivery_date,#cheque_date').datepicker({
    ...
    function calculate_alternate_value(item_id,rowcount,input_kg,input_block){
  // set_tax_value(rowcount);

   //Find the Tax type and Tax amount
   var operator_value = $("#tr_operator_value_"+rowcount).val();

  var input_qty     =$("#tr_input_value_"+rowcount).val();
  var input_block_value     =$("#tr_block_value_"+rowcount).val();
  //var gst_per         =$("#tr_item_per_"+rowcount).val();
  var item_qty        =$("#item_qty_"+item_id).val();
  

  var tot_block_kg_qty =parseFloat(input_block_value)*parseFloat(operator_value);
  //var gst_amt=(tot_sales_price * gst_per)/100;

  var item_qty        = parseFloat(tot_block_kg_qty)+parseFloat(input_qty)*1;
  var kg_qty = ( parseFloat(item_qty) % operator_value);

  console.log("kg_qty="+kg_qty);
  console.log("tot_block_kg_qty="+tot_block_kg_qty);
  // console.log("total_sales_qty="+total_sales_qty);
  console.log("item_qty="+item_qty);
  // final_total();
  $("#item_qty_"+item_id).val(item_qty)
  $("#qty_in_bg"+item_id+"_1").html(tot_block_kg_qty)
  $("#tr_input_value_"+item_id).val(kg_qty)
  $("#tr_block_value_"+item_id).val(tot_block_kg_qty)
  final_total();
}
$("#popup_retail_qty").keyup(function(){
  var input_qty = $(this).val();
  final_total();
  calculate_alternate_value(input_qty)
})
$("#popup_block_qty").keyup(function(){
  var black_qty = $(this).val();
  final_total();
  calculate_alternate_value(black_qty)
})
    // Sales Alternate Unit Operation Start
    function show_sales_alternate_unit_modal(row_id){

      $('#sales_unit').modal('toggle');

      //Find the item details
      var item_name = $("#td_data_"+row_id+"_0").html();
      var operator_value = $("#tr_operator_value_"+row_id).val();
      var input_qty = $("#tr_input_value_"+row_id).val();
      var black_qty = $("#tr_block_value_"+row_id).val();

      //Set to Popup
      $("#popup_item_name_al").html(item_name);
      $("#popup_operator_value").val(operator_value);
      $("#popup_retail_qty").val(input_qty);
      $("#popup_block_qty").val(black_qty);
      $("#popup_row_id").val(row_id);
    }
    function set_alt_info(){

    var row_id = $("#popup_row_id").val();
    var operator_value = $("#popup_operator_value").val();
    var input_qty = $("#popup_retail_qty").val();
    var black_qty = $("#popup_block_qty").val();

    //Set it into row 
    $("#tr_operator_value_"+row_id).val(operator_value);
    $("#tr_input_value_"+row_id).val(input_qty);
    $("#tr_block_value_"+row_id).val(black_qty);

    var item_id=$("#tr_item_id_"+row_id).val();
    calculate_alternate_value(item_id,row_id);
