...
<body onload="window.print();"><!--   -->
    ...
    // $reference_no = $this->db->query("SELECT reference_no from db_salesitems where sales_id='$sales_id'")->row()->reference_no;
    $payment_code = $this->db->query("SELECT payment_code from db_sales where id='$sales_id'")->row()->payment_code;
			...
			<td>
				...
				<table width="100%" cellpadding="0" cellspacing="0"  >
					...
					<tr style="border-top-style: dashed;border-bottom-style: dashed;border-width: 0.1px;">
						<th style="font-size: 11px; text-align: left;padding-left: 2px; padding-right: 2px;"><?= $this->lang->line('sales_due_payment'); ?></th>
					</tr>
					</thead>
					...
					<?php if(change_return_status()) {
						...
						<?php 
						 $date = date('Y-m-d');
						//  echo "$date";
						$payment=$this->db->query("SELECT new_amt_paid from db_sales where id = '$sales_id'")->row()->new_amt_paid;?>
